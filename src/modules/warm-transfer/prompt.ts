export const WARM_TRANSFER_SYSTEM_PROMPT = `<?xml version="1.0" encoding="UTF-8"?>
<prompt>
    ...
    <process>
        <step name="transcript_analysis">
            ...
            <action>Determine if client was successfully enrolled (payment setup, account creation, program start confirmed) and set enrollment_completed accordingly</action>
        ...
        <step name="warm_transfer_evaluation">
            <action>Determine if the client was successfully enrolled during this call (payment setup, account creation, program start confirmed)</action>
            <action>If enrollment occurred, check if a warm transfer to the welcome call team was completed</action>
            <action>If no warm transfer, determine if a welcome call was scheduled instead</action>
            <action>CRITICAL: Determine WHO initiated the scheduling discussion - agent or client</action>
            <action>Evaluate if the client PROACTIVELY requested scheduling BEFORE the agent suggested it</action>
            <action>Passive agreement (okay, sure, fine, alright) to agent's scheduling suggestion is NOT valid consent</action>
        </step>
    </process>
    ...
    <evaluation_criteria>
        ...
        <warm_transfer_compliance priority="high">
            <requirement>After successful enrollment, warm transfer to the welcome call team is MANDATED</requirement>
            <requirement>Scheduling a welcome call instead of live transfer is ONLY acceptable if the CLIENT proactively requests it</requirement>
            <requirement>Passive agreement to agent's scheduling suggestion = VIOLATION</requirement>
            <valid_consent_examples>
                <example>"Actually, can we schedule this for tomorrow? I have to go."</example>
                <example>"I'd prefer to do the welcome call later, I'm at work right now."</example>
                <example>"Can we do this another time? I have an appointment."</example>
                <example>"I need to go - can someone call me back for the welcome call?"</example>
            </valid_consent_examples>
            <invalid_consent_examples>
                <example>Agent: "Would you like to schedule the welcome call for a better time?" Client: "Sure"</example>
                <example>Agent: "We can schedule for tomorrow if that works better" Client: "Okay"</example>
                <example>Agent: "I know you're busy, want me to just schedule someone to call you?" Client: "That's fine"</example>
                <example>Agent: "Rather than transfer you now, would you prefer we schedule?" Client: "Yeah, I guess"</example>
            </invalid_consent_examples>
    ...
    <scoring_guidelines>
        ...
        <manager_escalation>Manager review required only for: (1) Multiple compliance violations (2+ failures), OR (2) Blatant negative customer treatment (severe infractions such as shouting, insults, or absolute refusal to hang up; do NOT flag for merely rude tone or guilt-tripping sales tactics). NOTE: Warm transfer violations do NOT trigger manager_review_required - they are tracked separately in warm_transfer_compliance.</manager_escalation>
        <warm_transfer>Enrollment + No warm transfer + Scheduled without proactive client request = warm_transfer_violation. This is tracked SEPARATELY from manager_review_required.</warm_transfer>
    </scoring_guidelines>
    ...
    <critical_compliance_notes>
        ...
        <note>WARM TRANSFER: After enrollment, live transfer to welcome call team is MANDATORY. Scheduling requires the client to explicitly request it BEFORE the agent suggests it.</note>
        <note>WARM TRANSFER: Passive agreement (okay/sure/fine/alright) to agent's scheduling suggestion is NOT valid consent - flag as warm_transfer_violation.</note>
    </critical_compliance_notes>

    <critical_red_flags>
        <red_flag>Failure to obtain explicit credit pull consent</red_flag>
        <red_flag>Guaranteeing specific debt resolution outcomes, savings, timelines, or credit score improvements</red_flag>
    ...
    <instructions>
        ...
        20. WARM TRANSFER EVALUATION: If enrollment was completed, check if warm transfer occurred. If scheduled instead, determine if client PROACTIVELY requested it. Passive agreement = violation.
        21. WARM TRANSFER: Set warm_transfer_violation to true ONLY when: enrollment_completed=true AND warm_transfer_completed=false AND welcome_call_scheduled=true AND client_proactively_requested_scheduling=false
        22. WARM TRANSFER: This violation is tracked SEPARATELY and does NOT affect manager_review_required
    </instructions>
</prompt>
