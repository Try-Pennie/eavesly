...
litigationCheckRoutes.post("/evaluate/litigation-check", async (c) => {
  ...
  const instanceId = `${callData.call_id}-litigation_check`

  try {
    const instance = await c.env.EVALUATION_WORKFLOW.create({
      id: instanceId,
      params: { moduleName: MODULE_NAMES.LITIGATION_CHECK, callData, correlationId },
    })
    ...
    const instances = await Promise.all(
      calls.map(callData =>
        c.env.EVALUATION_WORKFLOW.create({
          id: `${callData.call_id}-litigation_check`,
          params: { moduleName: MODULE_NAMES.LITIGATION_CHECK, callData, correlationId },
        })
      )
    )
