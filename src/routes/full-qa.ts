...
fullQARoutes.post("/evaluate/full-qa", async (c) => {
  ...
  const instanceId = `${callData.call_id}-full_qa`

  try {
    const instance = await c.env.EVALUATION_WORKFLOW.create({
      id: instanceId,
      params: { moduleName: MODULE_NAMES.FULL_QA, callData, correlationId },
    })
